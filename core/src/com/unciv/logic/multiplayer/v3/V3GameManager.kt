...
import com.unciv.logic.multiplayer.v3.net.V3GameClient
import com.unciv.logic.multiplayer.v3.net.V3GameHost
...
/**
 * EXPERIMENTAL / PREVIEW — the single UI-facing entry point for the multiplayer-v3 (authoritative,
 * command-in / filtered-view-out) netcode (docs/multiplayer-v3.md §4, §7). It owns the v2 game
 ...
 *    room, wraps the canonical [GameInfo] in a [V3GameHost] (which owns a [GameSession]) and starts
 ...
 *  - [joinGame] connects, joins the room, wraps a [V3GameClient] around a [ClientGameView] and
 ...
 *
 * ## Identity model (matches V3GameHost / V3GameClient)
 ...
 * instance is held by [UncivGame] for the duration of the v2 game (see [UncivGame.v3GameManager]).
 */
class V3GameManager {
    ...
    var host: V3GameHost? = null
        private set

    /** Player side, set only in client mode. */
    var client: V3GameClient? = null
        private set
    ...
    /**
     * EXPERIMENTAL / PREVIEW (multiplayer-v3): the tile of the Great Prophet currently founding /
    ...
    /**
     * HOST a v2 game over the relay. Connects, creates the room, wraps [gameInfo] in a [V3GameHost]
    ...
    suspend fun hostGame(
        ...
        // Tap inbound only long enough to await the Welcome (the room id); V3GameHost.start() then
        ...
        val h = V3GameHost(t, gameInfo, roster, hostUserId) { playerId, frame ->
        ...
        host = h
        Log.debug("V3GameManager: hosting v2 game in room %s as %s", welcome.roomId, hostUserId)
    ...
    private fun deliverLocal(frame: GameFrame) {
        when (frame) {
            ...
            is GameFrame.CommandRejected ->
                Log.debug("V3GameManager: host's own command rejected (seq %s): %s", frame.seq, frame.reason)
            else -> Unit
        }
    }
     ...
     * (so the host-naming `Welcome` is not missed), joins the room, and starts a [V3GameClient]
    ...
    suspend fun joinGame(
        ...
        clientView = view
        t.connect()
        val c = V3GameClient(t, myUserId = myUserId, view = view)
        ...
        this.roomId = roomId
        client = c
        Log.debug("V3GameManager: joined v2 room %s as %s", roomId, myUserId)
        return c
    }
     ...
     * [V3GameHost.submitLocal] (same validate/apply path, no socket round-trip). No-op (with a log)
    ...
    fun sendCommand(command: GameCommand) {
        ...
        val c = client
        if (c == null) {
            Log.debug("V3GameManager.sendCommand called with no host/client (not connected)")
            return
        }
     ...
     * pass-through to [V3GameClient.sendResyncRequest]; no-op (with a warning) if there is no client.
     *
    ...
    fun requestInitialView() {
        ...
        val c = client
        if (c == null) {
            Log.debug("V3GameManager.requestInitialView called with no host/client (not connected)")
            return
        }
