...
object CivilianUnitAutomation {
    ...
    fun automateCivilianUnit(unit: MapUnit, dangerousTiles: HashSet<Tile>) = timeThis<Unit>("automateCivilianUnit") {
        ...
        // (authority-only — the AI runs on the authority, no new command). A route-bound trade unit is on
        // autopilot — TurnManager shuttles it along its route — so the AI leaves it entirely alone.
        if (unit.isTradeUnit()) {
            ...
            if (manager.usedCapacity(unit.civ.civID) < manager.getMaxCapacity(unit.civ))
