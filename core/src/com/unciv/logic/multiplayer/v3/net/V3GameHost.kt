...
/**
 * The **authority side** of the multiplayer-v3 host loop: glues a [RelayTransport] to a
 * [GameSession] so a host runs the canonical simulation while remote players reach it through the
 * relay (docs/multiplayer-v3.md §4 — "the host runs `GameSession`; the relay routes opaque
 * frames").
 *
...
class V3GameHost(
     ...
     * every other player's frames still go over the relay. The owner ([V3GameManager]) supplies a
     ...
     * payload (docs/multiplayer-v3.md §8). Frames that carry no `playerId` are passed through
     * unchanged.
     */
