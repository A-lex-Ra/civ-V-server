...
/**
 * The **player side** of the multiplayer-v2 loop: glues a [RelayTransport] to a client-side view
 * holder so a remote player sends commands to the host and receives its own visibility-filtered
 * snapshots back, all over the relay (docs/multiplayer-v2.md §4).
 *
 ...
 *
 * ## Identity model
 * `PlayerId == UserId` (see [V2GameHost]). The client stamps its own [myUserId] into the
 * `playerId` field of the frames it sends; the host **ignores** that field and rebinds to the
 * connection id anyway (docs/multiplayer-v2.md §8), so the stamped value is only a courtesy /
 ...
 * non-empty peer list. (Host migration is deferred — docs/multiplayer-v2.md §7 — so a single latch
...
class V2GameClient(
    ...
    init {
        require((view == null) != (predictiveView == null)) {
            "V2GameClient needs exactly one of view / predictiveView"
        }
    }
     ...
     * connection identity regardless (docs/multiplayer-v2.md §8).
     *
