 ...
 * (docs/multiplayer-v3.md §4, §10). Combines the relay setup of [RelayIntegrationTest] (a real
 * [RelayServer] + ktor [WebSocketRelayTransport] clients) with the authority round-trip of
 * [GameSessionTest], wired together by [V3GameHost] / [V3GameClient]:
 *
...
class V3RelayIntegrationTest {
    ...
    fun clientCommandReachesHostAndFilteredViewComesBack() = runBlocking {
        ...
        // membership before driving the game (we do NOT read game frames out of it — V3GameHost owns
        ...
        try {
            // --- HOST: connect, create room, then hand the transport to V3GameHost ---
            ...
            val host = V3GameHost(hostTransport, testGame.gameInfo, roster, hostUserId)
            ...
            val client = V3GameClient(clientTransport, myUserId = clientUserId, view = clientView)
            ...
            // (PeerJoined arrives on the host; the host's V3GameHost handler ignores it, but the relay
