...
import com.unciv.logic.multiplayer.v3.V3GameManager
...
/**
 * EXPERIMENTAL / PREVIEW (multiplayer-v3, docs/multiplayer-v3.md §4/§7) — JOIN entry point.
 *
 ...
 *  1. [V3GameManager.joinGame] connects to the relay and joins the room.
 *  2. [V3GameManager.requestInitialView] asks the authority for an immediate filtered snapshot — the
 *     host only broadcasts on EndTurn, so a fresh joiner must ask or it would block until next turn.
 *  3. [V3GameManager.awaitFirstView] waits (with a timeout) for the first decoded filtered GameInfo.
 *  4. The manager is set on [UncivGame.v3GameManager] **before** [UncivGame.loadGame] so the new
 *     WorldScreen's init sees it (it checks `v3GameManager != null && !isHost` to wire its onView
 ...
 * manager down via [V3GameManager.close], leaving no half-open transport or dangling manager.
 */
class JoinV3GameScreen : PickerScreen() {
    init {
    ...
    private fun join(roomId: String) {
        ...
        Concurrency.run("JoinV2Game") {
            ...
            try {
                ...
                // Must be set BEFORE loadGame so WorldScreen.init sees it (reads v3GameManager + !isHost).
                UncivGame.Current.v3GameManager = manager
                ...
                if (UncivGame.Current.v3GameManager === manager) UncivGame.Current.v3GameManager = null
                launchOnGLThread {
